...
import { FiBell, FiTrash2, FiCheck, FiBriefcase, FiHeart, FiUser, FiActivity } from 'react-icons/fi'
import { Link } from 'react-router-dom'

const typeConfig = {
    application_status: { icon: <FiActivity />,  color: 'text-primary-hover bg-primary/10'  },
    job_recommendation: { icon: <FiBriefcase />, color: 'text-success bg-success/10'        },
    profile_update:     { icon: <FiUser />,      color: 'text-warning bg-warning/10'        },
    job_saved:          { icon: <FiHeart />,     color: 'text-danger bg-danger/10'          },
}
                    ...
                    <div className='flex flex-col gap-3'>
                        {notifications.length > 0 ? (
                            notifications.map((notif, idx) => {
                                const config = typeConfig[notif.type] || typeConfig.application_status
